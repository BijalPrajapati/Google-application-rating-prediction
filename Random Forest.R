rm(list=ls())
library("openxlsx")
data<-read.xlsx("C:\\Users\\Bijal\\Documents\\Final_Data_Both_Review_and_Information.xlsx")
...
a<-data[,c(2:8,13,14,15)]
results <- fastDummies::dummy_cols(a)
x<-results[,c(2:5,7,9,10,12:66)]

#set.seed(222)
ind <- sample(2, nrow(a), replace = T, prob = c(0.8, 0.2))
train <- a[ind==1,]
test <- x[ind==2,]
